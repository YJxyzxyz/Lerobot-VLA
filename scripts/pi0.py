"""
Deploy Trained pi_0 Policy in MuJoCo Simulation

Usage:
    python deploy_pi0.py --checkpoint ./ckpt/pi0_omy/checkpoints/last/pretrained_model
    python deploy_pi0.py --from_hub Jeongeun/omy_pnp_pi0
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Deploy pi_0 policy in simulation")
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="./ckpt/pi0_omy/checkpoints/last/pretrained_model",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Load model from HuggingFace Hub (e.g., Jeongeun/omy_pnp_pi0)"
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
...
def run_deployment(
    ...
    [Refactor 5] Encapsulate main loop logic into function with more state tracking
    """
    step = 0
    episode = 0
    ...
    while env.env.is_viewer_alive():
        ...
        if env.env.loop_every(HZ=control_hz):
            ...
            # [Refactor 6] Add progress printing
            if step % 50 == 0:
                print(f"  Step {step}...")
